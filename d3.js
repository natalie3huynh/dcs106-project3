...
// ── standalone, called from drawMap and the toggle ──
function redrawCanvas(ctx, width, height, projection, useCompare) {
  ...
  spherePathEl.setAttribute("d", pathGen({ type: "Sphere" }));
  ...
  for (let px = 0; px < width; px++) {
    for (let py = 0; py < height; py++) {
      ...
      let hex;
      if (useCompare && oceanData[region] && historicalData[region]) {
        const delta = oceanData[region].mean - historicalData[region].mean;
        hex = deltaColorScale(delta);
      } else {
        hex = regionColors[region];
      }
      if (!hex) continue;
  ...
  ctx.putImageData(imageData, 0, 0);  // ← paints the canvas
}
...
_height = height;
  ...
  // initial draw
  redrawCanvas(ctx, width, height, projection, false);

  // toggle lives here so it has access to ctx, width, height, projection
  let isCompareMode = false;
document.getElementById('toggle-btn').addEventListener('click', () => {
  isCompareMode = !isCompareMode;
  document.getElementById('toggle-btn').textContent =
    isCompareMode ? 'Show Present' : 'Show Δ Change';
  ...
  svg.append("path")
    ...
    .on("mousemove", function(event) {
      ...
      const present = oceanData[region];
      let html;

      if (isCompareMode && historicalData[region]) {
        const hist  = historicalData[region].mean;
        const mod   = present.mean;
        const delta = mod - hist;
        const sign  = delta >= 0 ? "+" : "";
        const color = delta >= 0 ? "#e85d24" : "#85b7eb";
        const arrow = delta >= 0 ? "▲" : "▼";
        html = `
          <div class="tooltip-title">${region}</div>
          <div class="tooltip-row">
            <span class="label">Historical mean</span>
            <span class="value">${hist.toFixed(2)}°C</span>
          </div>
          <div class="tooltip-row">
            <span class="label">Present mean</span>
            <span class="value">${mod.toFixed(2)}°C</span>
          </div>
          <div class="tooltip-row">
            <span class="label">Change</span>
            <span class="value" style="color:${color};font-weight:600">
              ${arrow} ${sign}${delta.toFixed(2)}°C
            </span>
          </div>`;
      } else {
        html = `
          ...
          <div class="tooltip-row">
            ...
            <span class="value mean">${present.mean.toFixed(1)}°C</span>
          </div>
          <div class="tooltip-row">
            <span class="label">Min</span>
            <span class="value cold">${present.min.toFixed(1)}°C</span>
          </div>
          <div class="tooltip-row">
            <span class="label">Max</span>
            <span class="value hot">${present.max.toFixed(1)}°C</span>
          </div>`;
      }
